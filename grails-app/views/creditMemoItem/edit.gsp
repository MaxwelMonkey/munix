      ...
      $(document).ready(function(){
        ...
        $('#salesDeliverySelect').change(function(){
            $.ajax({ url: '<g:createLink action="updateSelect"/>',
                data: "selectedValue="+$('#salesDeliverySelect').val()+"&creditMemoItemId="+${creditMemoItemInstance?.id}+"&creditMemoId="+${creditMemoItemInstance.creditMemo?.id},
        ...
        $('#salesDeliveryItems').change(function(){
            $.ajax({ url: '<g:createLink action="updateOldPriceAndQtyFields"/>',
                data: "selectedValue="+$('#salesDeliveryItems').val()+"&creditMemoItemId="+${creditMemoItemInstance?.id},
                success: function(resp){
                    $('#oldQty').val(resp.qty)
...
<body>
  ...
  <div class="body">
    <h1><g:message code="default.edit.label" args="['Credit Memo Item']" /></h1>
    ...
    <g:form method="post" >
      ...
      <g:hiddenField class="discount" name="discount" id="discount" value="${discount}"/>
      <g:hiddenField name="id" value="${creditMemoItemInstance?.id}" />
      <g:hiddenField name="version" value="${creditMemoItemInstance?.version}" />
      <div class="dialog">
        <table>
          <tbody>
          ...
          <g:link controller="creditMemo" action="show" id="${creditMemoItemInstance?.creditMemo?.id}">${creditMemoItemInstance?.creditMemo}</g:link>
          </td>
          </tr>
          ...
          <g:select name="deliveryItem.id" id="salesDeliveryItems" noSelection="${['':'Select One...']}" from="${salesDeliveryItemList}" optionKey="id" optionValue="product" value="${creditMemoItemInstance?.deliveryItem?.id}"  />
          </td>
          </tr>

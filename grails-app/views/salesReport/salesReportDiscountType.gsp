      ...
      <table>
        <thead>
          ...
          <th onclick="sortReport('date')">Date</th>
          	<g:each in="${discTypes}" status="k" var="dType">
	        	<th onclick="sortReport('${dType}')">${dType }</th>
	         </g:each>
        ...
        <tbody>
    	    		...
    	    		<g:set var="rowTotal" value="${0}"/>
	       			...
	       			<g:each in="${discTypes}" status="k" var="dType">
	       				...
	       				<g:set var="totalList" value="${0}"/>
	       					<g:each in="${byDateList.get(bean).get(dType.identifier)}" status="h" var="objects">
	       							...
	       							<g:each in="${objects}" status="r" var="object">
	       								<g:if test="${object!=null}">
	       									<g:if test="${object.status!='Cancelled'}">
		        								...
		        								<g:if test="${grandTotal.get(dType.identifier)!=null}">
		        									<% grandTotal.put(dType.identifier,  grandTotal.get(dType.identifier) + object.computeTotalAmount()) %>
		        								</g:if>
		        								<g:else>
		        									<% grandTotal.put(dType.identifier,  object.computeTotalAmount()) %>
		        								</g:else>
		        							</g:if>
	       								</g:if>
	       							</g:each>
            ...
            <g:if test="${params.totalHidden != 'Y'}">
	  		 <tr>
          		...
          		<g:each in="${discTypes}" status="k" var="dType">
          			<g:if test="${grandTotal.get(dType.identifier)!=null }">
	        			<td  class="right"><g:formatNumber number="${grandTotal.get(dType.identifier)}" format="###,##0.00" /></td>
	         		</g:if>
	         		<g:else>

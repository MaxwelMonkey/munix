...
<body>
  ...
  <div class="body">
    ...
    <g:hiddenField name="supplierId" id="supplierId" value="${purchaseOrderInstance?.supplier?.id}"/>
      <div class="dialog">
        <table>
          <tbody>
          <tr class="prop">
        ...
        <g:form>
          <g:hiddenField name="id" value="${purchaseOrderInstance?.id}" />
			<g:ifAnyGranted role="ROLE_MANAGER_PURCHASING,ROLE_PURCHASING">
              	<span class="button"><g:actionSubmit class="save" action="update" value="Update" onclick="isDirty=false;return confirm('${message(code: 'default.button.markAsComplete.confirm.message', default: 'Are you sure?')}');" /></span>
           	</g:ifAnyGranted>
        </g:form>
    </g:form>
  </div>
